...
// Variables to store MPU-9150 acc data
int mpu9150_accX;
int mpu9150_accY;
int mpu9150_accZ;

// Variables to store MPU-6050 acc data
float mpu6050_accX;
float mpu6050_accY;
float mpu6050_accZ;
...
{      
  ...
  // Configures the device as a BLE server
  ...
  // Creates a characteristic BLE to send data
  ...
  // Creates a characteristic BLE to receive data
  ...
  // Starts the discovery of the device
...
void loop()
{
  if (deviceConnected) {
    ...
    mpu6050_accX = (Wire.read() << 8 | Wire.read()) / 4096.0; // X-axis value
    mpu6050_accY = (Wire.read() << 8 | Wire.read()) / 4096.0; // Y-axis value
    mpu6050_accZ = (Wire.read() << 8 | Wire.read()) / 4096.0; // Z-axis value
    ...
    sprintf(mpuDataString,"%d,%d,%d,%.2f,%.2f,%.2f",mpu9150_accX,mpu9150_accY,mpu9150_accZ,mpu6050_accX,mpu6050_accY,mpu6050_accZ);
  
    pCharacteristic->setValue(mpuDataString);
       
    pCharacteristic->notify(); // Sends the value to the connected device
    Serial.print("*** Sent: ");
